...
export default function Applyinfo(props) {
  ...
  useEffect(() => {
    ...
    const fetchCompany = async () => {
      try {
        const response = await fetch(`https://mernbackend-4-19g0.onrender.com/api/students/job/${companyId}`, {
          method: "GET",
          credentials: 'include',
          headers: {
  ...
  const handleClick = async (e, companyId) => {
    try {
      ...
      const response = await fetch(`https://mernbackend-4-19g0.onrender.com/api/applications`, {
        method: "POST",
        credentials: 'include',
        body: formData
      });
